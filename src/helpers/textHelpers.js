...
export const singularToPlural = (text) => {
    if(isPlural(text)){
        ...
        if(new RegExp(/\_/ig).test(text))
            return text.split('_').map((textPart)=>`${textPart}${tablesPluralSuffix}`).join('_')
...
export const pluralToSingular = (text) => {
    if(isSingular(text)){
        ...
        if(new RegExp(/\_/ig).test(text))
            return text.split('_').map((textPart)=>`${textPart.substr(0,textPart.length-1)}`).join('_')
...
export const firstToUpperCase = (text) => {
    if(new RegExp(/\_/ig).test(text)){
        ...
        text.split('_').forEach((textPart)=>{
            res.push(textPart.split('').map((char,index)=>index===0 ? char.toUpperCase() : char ).join(''))
        })
        return res.join('_')
    }
