        ...
        tablesMutationsMethods.forEach(mutationMethod => {
            switch (mutationMethod) {
                case "insert": {
                    ...
                    if (isActionAllowed) {
                        //if is not table excluded from schema
                            ...
                            if (isActionAllowed) {
                                let statementResult = await db.exec(
                                    db
                                        .insert()
                                        .into(tableName)
                                        .setFieldsRows(args[tableName])
                                );

                                if (statementResult) {
                                    if (context.pubsub && statementResult.insertId > 0) {
                                        ...
                                        if (rowsInseret && Array.isArray(rowsInseret)) {
                                            let relationsFields = tableDesc.filter(item =>
                                                new RegExp(/\_/gi).test(item.Field)
                                            );
                                            ...
                                            context.pubsub.publish(
                                                `${tablesSubscriptionsMethods["insert"]}${firstToUpperCase(tableName)}`,
                                                {
                                                    [`${tablesSubscriptionsMethods["insert"]}${firstToUpperCase(
                                                        tableName
                                                    )}`]: recursiveStatementResult
                                                }
                                            );
                                        } else {
                            ...
                            if (isActionAllowed) {
                                ...
                                if (rowsToUpdate && Array.isArray(rowsToUpdate)) {
                                    ...
                                    if (statementResult) {
                                        if (
                                            context.pubsub &&
                                            statementResult.changedRows &&
                                            statementResult.changedRows > 0
                                        ) {
                                            //send to subscriptions
                                            let updatedRows = await db.exec(
                                                db
                                                    .select()
                                                    .from(tableName)
                                                    .where("id IN ?", rowsToUpdate.map(row => row.id))
                                            ); //to take the same rows just as with upgraded fields
                                            let relationsFields = tableDesc.filter(item =>
                                                new RegExp(/\_/gi).test(item.Field)
                                            );
                                            let recursiveStatementResult = await getRecursiveRelationTables(
                                            ...
                                            context.pubsub.publish(
                                                `${tablesSubscriptionsMethods["update"]}${firstToUpperCase(tableName)}`,
                                                {
                                                    [`${tablesSubscriptionsMethods["update"]}${firstToUpperCase(
                                                        tableName
                                                    )}`]: recursiveStatementResult
                                                }
                                            );
                                ...
                                if (rowsToDelete && Array.isArray(rowsToDelete)) {
                                    ...
                                    if (statementResult) {
                                        if (
                                            ...
                                            context.pubsub.publish(
                                                `${tablesSubscriptionsMethods["delete"]}${firstToUpperCase(tableName)}`,
                                                {
                                                    [`${tablesSubscriptionsMethods["delete"]}${firstToUpperCase(
                                                        tableName
                                                    )}`]: recursiveStatementResult
                                                }
                                            );
