...
const tablesValidator = (tables) => {
    ...
    tables.forEach((tableObject) => {
        ...
        if(new RegExp(/\_/ig).test(tableName) && tableName.split('_').filter((tableNamePart)=>!isPlural(tableNamePart)).length>0)//if is a relation table  && table name is not at plural
